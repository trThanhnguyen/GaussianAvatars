"""
Filename: evaluation_functions.py
    ...
    for bi, batch in enumerate(tqdm(dl)):
        ...
        for b in range(B):
            ...
            for key in flame_dict.keys():
                print(f'{key}: {flame_dict[key].shape}')

            flame_path = flame_folder / (f"flame_{suffix[1:]}.pkl")
            print(f'location of flame: {flame_path}')
...
def read_audio(audio_path):
    ...
    try:
        ...
        import soundfile as sf
        wavdata, sampling_rate = sf.read(audio_path, channels=1, samplerate=sampling_rate,dtype=np.float32, subtype='PCM_32',format="RAW",endian='LITTLE')
    # wavdata, sampling_rate = librosa.load(audio_path, sr=sampling_rate)
    if wavdata.ndim > 1:
        wavdata = librosa.to_mono(wavdata)
    wavdata = (wavdata.astype(np.float64) * 32768.0).astype(np.int16)
    # if longer than 30s cut it
    if wavdata.shape[0] > max_len * sampling_rate:
        wavdata = wavdata[:max_len * sampling_rate]
        print(f"!!WARNING: Audio longer than {max_len}s, cutting it to 30s")
